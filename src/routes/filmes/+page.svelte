<script>
    ...
    let pesquisa = ''; // Variável para armazenar o texto de pesquisa
  
    // Função para filtrar filmes pelo título
    function filtrarPorTitulo() {
        // Filtra os filmes com base no título, ignorando maiúsculas e minúsculas
        filtrados = filmes.filter(filme =>
            filme.título.toLowerCase().includes(pesquisa.toLowerCase())
        );
    }
...
<div class="row align-items-center mb-3">
    <div class="col">
        <input class="form-control" placeholder="Filtrar por título..." bind:value={pesquisa} on:input={filtrarPorTitulo} />
    </div>
    {#each gêneros as gênero}
        <div class="col">
            <div class="form-check form-check-inline">
                <input on:input={filtrarGenero} class="form-check-input" type="checkbox" id={gênero} value={gênero} />
